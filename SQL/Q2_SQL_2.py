...
class HW2_sql():
    ...
    # GTusername [0 points]
    def GTusername(self):
        gt_username = "hgerami3"
        return gt_username
    
    # Part a.i Create Tables [2 points]
    def part_ai_1(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_ai_1_sql = "CREATE TABLE movies(id integer,title text,score integer);"
        ######################################################################
    ...
    def part_aii_1(self,connection,path):
        ...
        #cursor.execute(".import movies.csv insert into movies")
        #a="import 'data/movies.csv' movies"
        #a = "COPY movies FROM 'movies.csv' DELIMITER ',';"
        #self.execute_query(connection, a)
       ######################################################################
        #with open(path,'r') as fin: # `with` statement available in 2.5+
        # csv.DictReader uses first line in file for column headings by default
        #dr = csv.DictReader(path) # comma is default delimiter
        #to_db = [(i['col1'], i['col2'], i['col3']) for i in dr]

        #cursor.executemany("INSERT INTO movies (col1, col2, col3) VALUES (?, ?, ?);", to_db)
        
        a_file=open("data/movies.csv",encoding="utf8")
        customer_data = csv.reader(a_file)
        #cursor.execute("CREATE TABLE movies(id integer,title text,score integer);")
        ...
        #cur.execute("INSERT INTO movies (id, title, score)" "VALUES (?, ?, ?)", row)
    ...
    def part_aii_2(self,connection, path):
        ############### CREATE IMPORT CODE BELOW ############################
        cur= connection.cursor()
        a_file=open("data/movie_cast.csv",encoding="utf8")
        customer_data = csv.reader(a_file)
        for row in customer_data:
    ...
    def part_c(self,connection):
        ...
        part_c_sql = """
                    SELECT
                    printf ('%.2f',((SELECT COUNT(*) FROM movies WHERE score > 50 and lower(title) LIKE '%war%')*1.00)/
                    (SELECT Count(*)FROM movies)*100.0)
                    """
    ...
    def part_d(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_d_sql = """
                    SELECT cast_name,COUNT(cast_name) as appearance_count
                    FROM movie_cast
                    WHERE  popularity >10
                    GROUP BY cast_name
    ...
    def part_e(self,connection):
        ...
        part_e_sql = """
                    SELECT 
                        ...
                        printf ('%.2f',movies.score) as movie_score,
                        DT.cast_count
                    from movies
                    INNER JOIN
                    (SELECT movie_id, count(*)as cast_count from movie_cast GROUP BY movie_id) DT
                    ON movies.id=DT.movie_id
                    
                    ORDER BY cast(movie_score as float)  DESC, cast_count ASC, movie_title ASC
                    LIMIT 5;
    ...
    def part_f(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_f_sql = """
        select cast_id, cast_name, average_score 
        FROM (select JMC.cast_id, JMC.cast_name, count(JMC.movie_id) as movie_count, printf ('%.2f',avg(JMC.score)) as average_score 
        From (select * from movie_cast MC left join movies M
        on M.id = MC.movie_id
        where M.score >= 25) JMC group by JMC.cast_name
                        having movie_count > 2
                        order by cast(average_score as float) desc, cast_name asc
                        Limit 10
                        );
        """
        ######################################################################
    ...
    def part_g(self,connection):
        ...
        part_g_sql = """
                    ...
                    select cast_member_id1, cast_id as cast_member_id2,count(*) as movie_count, avg(JMC.score)as average_movie_score   FROM
                    movie_cast MC
                    left JOIN
                    (select movie_id as id, cast_id as cast_member_id1, score from movie_cast MC
                    left join movies M
                    on M.id = MC.movie_id) JMC
                    on MC.movie_id=JMC.id and MC.cast_id<>JMC.cast_member_id1  
                    Where MC.cast_id IS NULL OR MC.cast_id < JMC.cast_member_id1
    ...
    def part_gi(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_g_i_sql = """
                select DISTINCT cast_id, cast_name, collaboration_score  from
                ...
                UNION 
                SELECT cast_member_id2, average_movie_score  FROM good_collaboration)
                GROUP BY cast_id) GC
                INNER JOIN 
                (select cast_id as member_id, cast_name from movie_cast )MC
                on MC.member_id = GC.cast_id
                ORDER BY cast(collaboration_score as float) DESC,
                            cast_name ASC
                Limit 5;        
    ...
    def part_h(self,connection,path):
        ############### EDIT SQL STATEMENT ###################################
        part_h_sql = """
        CREATE VIRTUAL TABLE IF NOT EXISTS movie_overview USING fts4(id INT, overview TEXT)
        """
        ######################################################################
        connection.execute(part_h_sql)
        ############### CREATE IMPORT CODE BELOW ############################
        with open(path, encoding='utf-8') as f:
            cast_csv = csv.reader(f)
            for row in cast_csv:
                connection.execute("INSERT INTO movie_overview (id, overview) VALUES(?,?);",tuple(row))
            connection.commit()
        ######################################################################
